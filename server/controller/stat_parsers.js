module.exports = {
    ...
    switch (dex) {
      ...
      case 9:
        return {
          ...
          missile: 0,
          ac: 0,
          thief_adj: [0, -15, 0, -10, -20, -10, -15, 0]
        };
      case 10:
        return {
          suprise: 0,
      ...
      case 19:
        return {
          ...
          missile: 3,
          ac: -4,
          thief_adj: [0, 15, 0, 15, 15, 20, 15, 0]
        };
    }
  },
  con: (con, classes) => {
    if (
      classes.includes("Fighter") ||
      classes.includes("Paladin") ||
      (classes.includes("Ranger") && con > 16)
    ) {
      switch (con) {
        case 17:
  ...
  dualClassParser: character => {
    ...
    character.savingThrows = ogClass.saving_throws.map((num, i) => {
      return (
        Math.min(num, newClass.saving_throws[i]) - character.saving_throw_adj
      );
    });
    ...
    ogHP =
      ogClass.hp.reduce((acc, num, i) => {
        if (i < ogClass.level) return acc + num + character.con.hp;
      }, 0) + ogClass.hp_bonus;
    newHP =
      newClass.hp.reduce((acc, num, i) => {
  ...
  multiClassParser: character => {
    ...
    character.classDetails.forEach(cLass => {
      ...
      savingThrows = savingThrows.map((num, i) => {
        return Math.min(
          num,
          cLass.saving_throws[i] - character.saving_throw_adj
        );
      });
      count++;
    });
  ...
  classParser: character => {
    character.hp =
      character.classDetails[0].hp.reduce((acc, num) => {
        return acc + num + character.con.hp;
      }) + character.classDetails[0].hp_bonus;
    character.savingThrows = character.classDetails[0].saving_throws.map(
      savingThrow => {
  ...
  racialSavingThrows: (character, con) => {
    character.resistances += `&+${Math.floor(
      con / 3.5
    )} to saves against poison and magic`;
    return character;
  },
