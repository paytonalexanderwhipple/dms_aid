...
module.exports = {
  read: async (req, res) => {
    ...
    const fullCharacterArray = await Promise.all(
      characterArray.map(async character => {
        let classDetails = [];
        await Promise.all(
          characterDetails.map(async cLass => {
            if (
              cLass.character_id === character.character_id &&
              cLass.campaign_id === character.campaign_id
            ) {
              let levelLimits;
              ...
              if (levelLimits) {
                ...
                if (cLass.level > levelLimit) {
                  const newLevel = await db.get_class_by_level_shallow([
                    cLass.class_name,
                    levelLimit
                  ]);
  ...
  create: async (req, res) => {
    ...
    const db = req.app.get("db");
    try {
      const newCampaign = await db.create_campaign([
        campaignName,
        text,
        levelLimits,
        description
      ]);
      await db.create_campaign_user([
        newCampaign[0].campaign_id,
  ...
  createJoin: async (req, res) => {
    ...
    if (!campaign) {
      res
        .status(404)
        .send(
          "That campaign does not exist. (yet), or you just mispelled the name."
        );
      return "";
    }
    const isParticipating = await db.check_for_campaign_participation([
      user_id,
      campaign.campaign_id
    ]);
    if (!isParticipating[0]) {
      await db.create_join_invite([
        campaign.campaign_id,
        user_id,
        "join",
        text
      ...
      if (isParticipating[0].is_dm) {
        res
          .status(409)
          .send(
            "You are already participating in this campaign. In fact you are incharge."
          );
      } else {
  ...
  getCurrentCampaign: async (req, res) => {
    ...
    const characterData = await db.get_current_characters([
      campaign_id,
      user_id
    ]);
    ...
    const characterDetails = await Promise.all(
      characterData.map(async character => {
        ...
        for (cLass of classData) {
          ...
          if (cLass.character_id === character.character_id) {
            ...
            if (campaignData[0].level_limits) {
              levelLimit = stat_parsers.checkLevelLimit(character, cLass);
              if (cLass.level > levelLimit) {
                const newLevel = await db.get_class_by_level([
                  cLass.class_name,
                  levelLimit
                ]);
        ...
        weapons.forEach(weapon => {
          if (weapon.character_id === character.character_id)
            weaponDetails.push(weapon);
        });
        const armorDetails = [];
        armor.forEach(armor => {
          if (armor.character_id === character.character_id)
            armorDetails.push(armor);
        });
        const itemList = [];
        items.forEach(item => {
        ...
        character.str = stat_parsers.str(
          str,
          character.classes,
          execptionalStrength
        );
